...
class Spaz(bs.Actor):
    ...
    def __init__(
        ...
        *,
        ...
        if is_killer:
            ...
            playermat = AsymFactory.get().survivor_material
        punchmats = (factory.punch_material, shared.attack_material)
        pickupmats = (factory.pickup_material, shared.pickup_material)

        
        materials.append(playermat)
        roller_materials.append(playermat)
        extras_material.append(playermat)
        punchmats += (playermat,)
        pickupmats += (playermat,)
        ...
        self.invincble_shield = bs.Node(None)
        ...
        try:
            self.moveset=bs.app.classic.spaz_appearances[character].moveset(self)
        except Exception as e:
            print('Unable to apply moveset. ', e)
        self.character = character

        

        if self.moveset:
            self.max_walk_speed = self.moveset.move_speed
            self.max_run_speed = self.moveset.run_speed
            self.hitpoints = self.moveset.hitpoints * 10
            self.hitpoints_max = self.moveset.hitpoints * 10
        else:
    ...
    def on_pickup_release(self) -> None:
        ...
        """
        if self.moveset:
            # Moveset exists, dont do anything.
            return
        if not self.node:
            return
    ...
    def set_invincible(self, duration):
        # Already invincible, cant stack.
        if not self.is_alive():
            return

        # set our thingies to false
        self.node.invincible = True
        self.invincble_shield.delete()
            
        self.invincble_shield = bs.newnode(
                    'shield',
                    owner=self.node,
                    attrs={'color': (0.3, 0.2, 2.0), 'radius': 1.3},
                )
        self.node.connectattr('position_center', self.invincble_shield, 'position')
        try:
            bs.timer(duration, lambda: (
                self.invincble_shield.delete(),
                setattr(self.node, 'invincible', False)
            ))
            # node porlly got deleted
        except:
            pass
            

    def speed_boost(self, dur):
        if not self.node:
            return
    ...
    def handlemessage(self, msg: Any) -> Any:
        ...
        if isinstance(msg, DamageMessage):
            if not self.is_alive():
                return
            
            # Were invincible, no-op any damage.
        ...
        elif isinstance(msg, StunMessage):
            if not self.node:
                return
            # Were invincible, no-op any stuns.
            if self.node.invincible:
                SpazFactory.get().block_sound.play(
                    1.0,
                    position=self.node.position,
            ...
            if msg.spaz:
                if msg.spaz.moveset:
                    msg.spaz.moveset.handle_spaz_hit_stun(msg.hittype)
            # Also tell OUR moveset we were stunned.
            if self.moveset:
                self.moveset.handle_spaz_was_stunned(msg.hittype)
            ...
            # We got some knockback settings!
            if bool(msg.knockback_settings):
               
                self.impulse(
                    x=msg.knockback_settings['x'],
                    y=msg.knockback_settings['y'],
                    direction=msg.knockback_settings['direction'],
                )
            ...
            if msg.immediate:
                ...
                if not wasdead:
                    if self.source_player and msg.how is not bs.DeathType.LEFT_GAME:
                        self.getactivity().handlemessage(bs.PlayerDiedMessage(
                            self.source_player, True, None, msg.how
                        ))
                    # if we got a moveset, delete the text nodes
                    if self.moveset:
                        for nodes in self.moveset._ability_ui_nodes:
                            nodes['icon'].delete()
                            nodes['timer'].delete()
                        self.moveset._ability_ui_nodes = {}
                    self.node.hurt = 1.0
            ...
            if self.moveset:
                if not self.moveset.handle_spaz_punched_something(collision):
                    # The moveset asked not for us to handle anything, so bye.
                    return
